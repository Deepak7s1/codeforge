...
public class TrustServiceUtil
{
	...
	public static Token issueSecurityToken(String userName) throws Exception
	{
		 ...
		 * javax.security.auth.Subject subject = SubjectUtil.getCurrentSubject();
		 * userName = SubjectUtil.getUserName( subject );
		 * 
		 */
	...
	public static String getSecurityToken(String userName) throws Exception
	{
		try
		{
			Token token = TrustServiceUtil.issueSecurityToken(userName);
			
			String encodedToken = TrustServiceUtil.encodeSecurityToken( token );
			
			return encodedToken;
			
			// method.addRequestHeader(AUTHORIZATION, AUTH_TYPE_NAME + SPACE +
			// encodedToken);
